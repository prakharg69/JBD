 ...
 * Defined with the function keyword at statement level. 
 * The entire function body is hoisted, so you can call it before it appears in code.
 */
...
/* ✓ Production tip: use declarations for all top-level, reusable functions. 
 * They appear clearly in stack traces with their real name — anonymous functions 
 * show as "(anonymous)" and make debugging painful.
 */
...
/* ⚠️ Only declarations are fully hoisted. Function expressions and arrow 
 * functions assigned to const are NOT — calling them before assignment 
 ...
 * A function stored in a variable. Not hoisted. The variable exists but 
 * holds undefined until the line is reached.
 */
...
const calculateNamed = function calculateDouble(n) {
  return n * 2;
};
// calculateDouble appears in stack traces, not "(anonymous)"

/* ✓ Production tip: always give function expressions a name — 
 * write const fn = function myFn() {} not const fn = function() {}. 
 * Same debugging benefit as declarations.
 */
 ...
 * Shorter syntax introduced in ES6. Key difference: arrow functions do NOT 
...
/* ✓ Use arrow functions for callbacks, array methods, and anything that 
 * needs to inherit this. Use declarations/expressions for methods on 
 * objects or classes.
 */
...
/* ⚠️ Never use arrow functions as object methods or class methods. 
 * They don't bind this — so this will be the outer scope (usually 
...
/* Parameters are the names in the function definition. Arguments are the 
 * values you pass in. JS never throws for wrong argument count — missing 
...
/* ✓ Production tip: prefer object parameters over positional ones when a 
 * function takes 3+ arguments. It makes call sites self-documenting and 
 * order-independent — no more fn(true, false, null, 5) mysteries.
 */
...
/* ✓ Functions should always return a consistent shape. Mixing return value 
 * and return undefined makes callers write defensive code everywhere. 
 ...
 * Default parameters are evaluated at call time if the argument is undefined. 
 * They can reference earlier parameters and even call functions.
 */
...
/* ✓ Production tip: use defaults for optional config. Put required params 
 * first, optional (defaulted) params last. This lets callers omit trailing 
 * args cleanly: fn(required) instead of fn(required, undefined, undefined, option).
 */
...
function paginateWrong(page, size) {
...
/* ⚠️ Never use || to set defaults for numeric or boolean params — it coerces 
 * 0 and false. Always use real default parameters or ?? as fallback.
 */
 ...
 * The ...rest parameter collects all remaining arguments into a proper array. 
...
/* ✓ Rest parameters replace the old arguments object. Unlike arguments, 
 * rest params are a real array — you can use .map(), .filter() etc. on 
 * them directly. Never use arguments in modern code.
 */
...
/* ⚠️ arguments is not available in arrow functions. If you need variadic 
 * args in an arrow function, you MUST use rest params. Another reason to 
 * prefer rest everywhere.
 */
 ...
 * Spread (...) unpacks arrays or objects. It's the inverse of rest — 
...
/* ✓ Spread for cloning is shallow — nested objects are still references. 
 * For config merging, put the override last: { ...defaults, ...overrides }. 
 * Last key wins in object spread.
 */
...
/* ✓ The [...new Set(arr)] pattern is the cleanest one-liner to deduplicate 
 * arrays in production. It preserves insertion order and handles all 
 * primitive types.
 */

// --------------------------------------------
// REST VS SPREAD — COMPARISON TABLE
// --------------------------------------------
 ...
 * JS has three scope levels. Inner scopes can access outer variables, 
 * but not vice versa.
 */
...
/* ✓ Keep variables as close to where they are used as possible. Avoid 
 * global variables — they make testing and debugging much harder. 
 ...
 * When a function is created, it captures a reference to all variables 
 * in its surrounding scope — even after that scope finishes. This is a closure.
 */
...
/* ✓ Closure is the foundation of: private state, factory functions, 
 * memoisation, event handlers that remember context, and React hooks 
 * (useState is built on closure).
 */

/* It works because of closures, which allow functions to retain access 
 * to variables from their outer scope—even after that function has 
 * finished executing. Same for this.
 */
...
/* | Feature              | Syntax                                    | Hoisted | Own `this` |
 * |----------------------|-------------------------------------------|---------|-------------|
 * | Declaration          | `function fn() {}`                        | ✓ Yes   | ✓ Yes       |
 * | Expression (named)   | `const fn = function named() {}`          | ✗ No    | ✓ Yes       |
 * | Expression (anon)    | `const fn = function() {}`                | ✗ No    | ✓ Yes       |
 * | Arrow (implicit)     | `const fn = () => value`                  | ✗ No    | ✗ No        |
 * | Arrow (block)        | `const fn = () => { return value; }`      | ✗ No    | ✗ No        |
 */

/* | Pattern              | When to Use                               |
 * |----------------------|-------------------------------------------|
 * | Declaration          | Top-level reusable functions              |
 * | Arrow                | Callbacks, array methods, inline functions|
 * | Default params       | Optional configuration parameters         |
 * | Rest params          | Variadic functions (variable arguments)   |
 * | Spread               | Cloning, merging, Math.max() style calls  |
 * | Destructuring params | Functions with 3+ parameters              |
 * | Closure              | Private state, factories, memoisation     |
 */